...
@login_required
@admin_required # Admins podem excluir qualquer usuario
...
@app.route('/post/<int:post_id>/excluir_posts', methods=['POST'])
@login_required
@admin_required # Admins podem excluir qualquer post
def excluir_posts(post_id):
    post = Post.query.get(post_id)
    if post:
        database.session.delete(post)
        database.session.commit()
